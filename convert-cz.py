...
class OpenLiricsDocument:
    def __init__(self, xml):
        ...
        if self.verse_order is None:
            self.line_structure = dict(sorted(self.line_structure.items(), key=lambda x: x[1][0]))
...
def convert_in_range(target_folder, start, end):
    ...
    while i < end:
        try:
            ...
            if os.path.exists(target_filename):
                problematicFiles.append(to_convert[i]+ '    [Stejny nazev jako jina pisen]')
                i+=1
                continue
    ...
    if len(problematicFiles) > 0:
        print(eol+eol+"Mame tu problematicke soubory s kteri si nevime rady" )
        print(eol+"Format popisu je jmeno_souboru.xml    [problem]"+eol+eol)
...
if __name__ == '__main__':
    ...
    if filecount < 1:
        try:
            input("nenasli se zadne xml soubory... koncim")
        except KeyboardInterrupt:
            exit(0)
        import sys
        sys.exit(-1)
    print("Nasel jsem %d xml souboru" % filecount)
    import time
    def folder_writeable(path):
        ...
        try:
            ...
            print("Nemuzu zapisovat cist z/do adresare")
        except PermissionError:
            print("Na adresar nemam prava")
        if succes:
    ...
    try:
        out_folder = input(
            "vyberte adresar kam hodit prelozeny vysledek:").strip()
        if end_w_slash.search(out_folder) is None:
            out_folder += "/"

        while not folder_writeable(out_folder):
            out_folder = input("Zkus to znovu:").strip()
            if end_w_slash.search(out_folder) is None:
                out_folder += "/"
